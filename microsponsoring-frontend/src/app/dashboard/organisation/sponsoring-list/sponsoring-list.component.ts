...
@Component({
  ...
  styleUrl: './sponsoring-list.component.css'
})
export class SponsoringListComponent implements OnInit {
    ...
    if (user && user.userId) {
      this.companyService.getCompanyByUserId(user.userId).subscribe(company => {
        if (company && company.companyId) {
          this.paymentService.getInvoicesByCompany(company.companyId).subscribe((invoices: any[]) => {
            ...
            const sponsorIds = Array.from(new Set(invoices.map(inv => inv.sponsor?.sponsorId).filter(id => !!id)));
            if (sponsorIds.length > 0) {
              this.userService.getUsersBySponsorIds(sponsorIds).subscribe(usersMap => {
                for (const sponsorId in usersMap) {
                  const user = usersMap[sponsorId];
                  this.sponsorNames[sponsorId] = user.fullName || user.username || 'N/A';
                }
              });
            }
  ...
  getSponsorName(s: any): string {
    if (s.sponsor && s.sponsor.sponsorId) {
      return this.sponsorNames[s.sponsor.sponsorId] || 'Loading...';
    }
    return 'N/A';
  }
}